...
Спільний хелпер: лінива мапа "є значення за ключем — віддай, нема — створи дефолт і збережи" (`getOrCreate`), плюс звичайні `get`/`set`/`delete`. Виник тому, що цей патерн був буквально скопійований у 4 місцях (`ActiveWorldService`, `SharedModelService`, `WorldRepresentationService`, і раніше — камера всередині `WorldCanvasComponent`, відтоді винесена в `WorldCameraMemoryService`, див. нижче) — тепер він написаний один раз.

### `state/sessions.service.ts` — `SessionsService`
Список сесій (`Session[]` = `{id, name}`) + `activeSessionId: number | null`. **Стартовий стан не порожній** — конструктор одразу створює одну сесію (`signal<Session[]>([this.createSessionEntry()])`) і робить її активною; порожній список (0 сесій) — стан, у який можна лише *повернутись* закривши всі сесії, а не з чого застосунок стартує. `createSession()` додає сесію і одразу робить активною — необмежена кількість. `closeSession(id)` видаляє сесію зі списку; якщо закривалась активна — обирає сусідню (індекс `min(closingIndex, remaining.length - 1)`), а якщо сесій не лишилось — `activeSessionId` стає `null`. `id` не перевикористовується (`nextId` лише зростає) — навмисно, бо очистка `KeyedStore`-мап у нижчих сервісах іде через `effect()` асинхронно, і перевикористаний id міг би на мить дочитати чужі старі дані. Нічого не знає про World'и чи модель — це нижче.
...
**Важливо: ізоляція зараз лише per-сесія, НЕ per-World.** Ключ мапи — тільки `sessionId`; усі 3 World'и однієї сесії отримують **один і той самий** об'єкт (див. нижче — `WorldRepresentationService.getRepresentation` ігнорує `worldIndex` саме тому). Це розходиться із задуманою моделлю "Ideal World рахує, Real/Solver World мають власні координати, виведені реплеєм" (`docs/IDEAS.md`, R4, чернетка) — коли ця логіка реалізовуватиметься, `SharedModelService` (і, ймовірно, сам тип, що він зберігає — вже не голий `Object3D`, а щось на кшталт Mesh-даних) доведеться переключити на ключ `(sessionId, worldIndex)`. Додатково: `WorldCanvasComponent.updateModel()` переклоновує модель у сцену лише коли міняється **посилання** на `representation.object` (`this.lastModel !== object`) — мутація вмісту об'єкта "на місці" (без заміни посилання) не долетить до вже відкритих канвасів автоматично.
...
### Імпортована референс-геометрія — `ImportedGeometryService` / `ImportedReferenceDisplayService` / `ImportedReferenceRenderService`

Три `root`-scoped, `KeyedStore<sessionId, …>` сервіси (той самий патерн, що й вище) для геометрії, яку користувач імпортує в Ideal World (STL/GLB) як **референс** — візуальний орієнтир поруч із моделлю сесії, а не сама модель (`SharedModelService`) і не щось, що редагується. Розділені за трьома різними причинами змінюватись:

- **`state/imported-geometry.service.ts` — `ImportedGeometryService`.** Сира імпортована геометрія + метадані (`fileName`, `watertight`, `meshCount`/`triangleCount`/`vertexCount`, `boundingSize`/`boundingBox`/`longestAxis`). **`.object` — НЕ сам завантажений меш, а pivot `THREE.Group`, що його огортає.** `set()` спершу центрує локальну геометрію об'єкта відносно її ж власного origin, потім кладе її в `pivot` — так `pivot`-а локальна `(0,0,0)` збігається зі справжнім геометричним центром об'єкта (а не довільним origin, який лишив автор файлу), і рівно тому обертання (нижче) крутить об'єкт на місці, а не навколо випадкової точки. `pivot` додатково центрується по X/Z і "ставиться на підлогу" (`min.y = 0`) через `recenterAtOrigin` (`geometry/recenter-object3d.ts`). **Важливо для майбутніх споживачів** (напр. запланована воксилізація, `docs/IDEAS.md`): якщо колись знадобиться сира, непозиційна геометрія файлу як є — треба явно розгортати `pivot`, а не брати `.object` напряму.
- **`state/imported-reference-display.service.ts` — `ImportedReferenceDisplayService`.** Чистий стиль показу (`visible`, `mode: 'solid'|'wireframe'`, `color`, `opacity`, `dimensionsVisible`, `rulerVisible`, `rulerDistance`, `rotateGizmoVisible`) — жодної геометрії, лише УПОДОБАННЯ користувача щодо того, як малювати те, що вже є в `ImportedGeometryService`.
- **`state/imported-reference-render.service.ts` — `ImportedReferenceRenderService`.** Похідні, кешовані `THREE.Object3D` для сцени: масштабований (за "density") і повернутий (`getRotation`/`setRotation`, крутілка в `WorldCanvasComponent`) клон референсу, плюс лінії розмірів і рулетка (`geometry/dimension-lines.ts`/`geometry/ruler-preview.ts`), побудовані в тому самому локальному, pivot-центрованому кадрі, що й сам клон — тому досить скопіювати `position`+`quaternion` клону на них, щоб вони жорстко трималися об'єкта при повороті. **Кеш, не `computed`:** `getScaledReference`/`getDimensionLines`/`getRuler` віддають те, що востаннє поклав `refreshScaledReference`/`refreshRuler` — не будують нічого на льоту. Це навмисно: `WorldCanvasComponent` перебудовує свій клон у сцені лише коли міняється **посилання** на вхідний об'єкт (той самий принцип, що й `SharedModelService.updateModel`), і сервіс, що віддавав би свіжий `.clone()` на кожен виклик гетера, ламав би цю оптимізацію й тряс FPS щокадру. Ціна: інваріант "перебудуй усе, що залежить" тримається вручну (`refreshScaledReference` викликає `refreshRuler`, обидва мають бути викликані там, де щось із density/rotation/import змінилось) — не через реактивний граф Angular, як в інших сервісах цього розділу.

Усе UI цих трьох сервісів живе в `layout/settings-panel/imported-reference-controls/` — `ImportedReferenceControlsComponent`, винесений із `SettingsPanelComponent` (той самий мотив, що й раніше для `CadWorkspaceComponent`/`WorkspacePlaceholderComponent`: панель розросталась одним плоским файлом). Отримує `sessionId` через `@Input` від батька (той самий "батько рахує ідентичність один раз, дитина отримує через `@Input`" патерн, що й `WorldCanvasComponent.sessionId`), а не інжектить `SessionsService` сам. `SettingsPanelComponent` рендерить його лише за `*ngIf="isIdealWorld()"` і лишає собі: контекст сесії/World, секцію Import, і більше нічого про референс-геометрію.
...
3. **Кут камери — своя пам'ять на пару (World, сесія)**, попри спільний канвас: зберігається в `WorldCameraMemoryService` (root-scoped, не всередині `WorldCanvasComponent` — див. вище).
...
- **Перехід на іншу toolbar-іконку й назад перестворює всі 3 WebGL-канваси** — `CadWorkspaceComponent` (а з ним `RenderWindowComponent` і 3 `WorldCanvasComponent`) живе, лише поки активна іконка 0; `NgComponentOutlet` знищує його при переході на іншу іконку. "Рівно 3 WebGL-контексти за весь час роботи застосунку" (принцип 1 нижче) тепер правильний лише за умови, що користувач не виходить з іконки 0 — кут камери й скомпільовані шейдери губляться при **кожному** такому переході, не в окремих випадках: `WorldCanvasComponent.ngOnDestroy()` нічого не зберігає в `WorldCameraMemoryService` (лише `cancelAnimationFrame`/`dispose()`) — збереження кута відбувається виключно в `updateSession()` при зміні `sessionId`, якої тут не стається. Дані сесій/моделі при цьому не губляться (вони в root-сервісах, не в компоненті). Некритично, поки інші іконки — заглушки без реального use case, але як тільки з'явиться причина йти на іншу іконку й повертатись під час роботи з геометрією, це стане помітним постійним UX-подразником — вартий фіксу, коли дійде черга (варіант: підняти `RenderWindowComponent`/`WorldCanvasComponent` на рівень `AppComponent`, назавжди змонтовані, перемикати видимість CSS-ом, як уже зроблено для `SessionTabsComponent`).
