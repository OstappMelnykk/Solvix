...
// building them fresh from `grid` - both geometry/surface-projection.ts
// (snapping exterior corners onto the real STL surface) and
// geometry/voxel-simplify.ts (merging long straight runs into stretched
// boxes) need this: each produces its own VoxelCell[] whose corners no
// longer match a fresh buildVoxelCells(grid) call, so the preview has to
// be built from THAT SAME array, not rebuilt from the grid (which would
// silently discard whatever they did and fall back to plain lattice
// cells). VoxelizationService owns calling either transform (if at all)
// BEFORE this, then this, on the exact same cells.
